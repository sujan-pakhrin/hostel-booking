...
export const createUser = (req, res, next) => {
    ...
    const { f_name, l_name, address, email, phone, password, gender } =
        req.body;
                                ...
                                if (err) {
                                    ...
                                    if (
                                        ...
                                        const updatedData = {
                                            f_name:
                                                f_name ||
                                                existingUserData.f_name,
                                            l_name:
                                                l_name ||
                                                existingUserData.l_name,
                                            address:
                                                address ||
    ...
    db.query(sql, id, (err, data) => {
        ...
        bcrypt.compare(currentPassword, data[0].password, (err, isMatch) => {
            ...
            bcrypt.compare(newPassword, data[0].password, (err, isSame) => {
                ...
                if (isSame) {
                    return next(
                        errorHandler(400, "Please enter a different password")
                    );
                }
...
export const changeProfile = (req, res, next) => {
    ...
    db.query(sql, [profile, id], (err, data) => {
        if (err) {
            return next(
                errorHandler(500, "Something went wrong, please try later")
            );
        }
        res.send({
            success: 1,
                ...
                sendMail({
                    ...
                    .then((messageId) => {
                        ...
                        db.query(sql, [otp, email], (err, data) => {
                            if (err) {
                                return next(
                                    errorHandler(500, "Something went wrong!!")
                                );
                            } else {
                                res.status(200).send({
                                    message: "OTP send successfully",
                    ...
                    .catch((error) => {
                        return next(
                            errorHandler("Failed to send email:", error)
                        );
                    });
            }
        }
